...
export async function POST(request: NextRequest) {
  try {
    ...
    // Save to local database — soft failure on serverless (e.g., Vercel) where
    // SQLite is not persistent. GHL CRM is the primary data store.
    let dbSaved = false;
    try {
      ...
      console.error('[Rescue Lead] DB save failed (non-blocking):', dbErr);
      // Non-blocking — GHL CRM is the primary store; don't fail the request
    }
