...
const shieldPdfRateLimits = new Map<string, number>();
const SHIELD_PDF_RATE_LIMIT_MS = 60_000;

// Cleanup rate limits every 5 minutes
setInterval(() => {
  const cutoff = Date.now() - SHIELD_PDF_RATE_LIMIT_MS * 2;
  for (const [ip, ts] of shieldPdfRateLimits) {
    if (ts < cutoff) shieldPdfRateLimits.delete(ip);
  }
}, 300_000).unref();
...
export async function GET(_req: NextRequest) {
  try {
    ...
    const now = Date.now();
    if (!shieldPdfRateLimits.has(clientIp) || now - shieldPdfRateLimits.get(clientIp)! > SHIELD_PDF_RATE_LIMIT_MS) {
      shieldPdfRateLimits.set(clientIp, now);
    } else {
...
<body>
  ...
  <div class="page body-page">
    ...
    // Try Playwright PDF generation (works in local / standalone environments)
    // On Vercel serverless, Playwright is not available, so we serve styled HTML
    // with a print prompt that lets users save as PDF from the browser
    try {
      ...
      // Playwright not available (serverless / Vercel)
      // Return styled HTML that users can print/save-as-PDF from browser
    ...
    console.error('Shield PDF generation error:', error);
    return NextResponse.json(
      { error: 'Failed to generate the Shield document' },
      { status: 500 }
    );
  }
}
